...
static const char *load_gesture_with_id_sql = "SELECT id, points, description, "
                                              "command FROM gesture WHERE id = "
                                              "? LIMIT 1;";
...
Database *database_open(/* out */ const char **error) {
  ...
  *error = NULL;
  ...
  const int status = sqlite3_open_v2(
      dbpath, &db->db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL);
  free(dbpath);
  if (SQLITE_OK != status) {
  ...
  if (SQLITE_OK != sqlite3_prepare_v2(db->db, insert_gesture_sql, -1,
                                      &db->insert_gesture_stmt, NULL)) {
    *error = sqlite3_errstr(sqlite3_extended_errcode(db->db));
    goto error;
  }

  if (SQLITE_OK != sqlite3_prepare_v2(db->db, load_gestures_sql, -1,
                                      &db->load_gestures_stmt, NULL)) {
    *error = sqlite3_errstr(sqlite3_extended_errcode(db->db));
    goto error;
  }

  if (SQLITE_OK != sqlite3_prepare_v2(db->db, load_gesture_with_id_sql, -1,
                                      &db->load_gesture_with_id_stmt, NULL)) {
    *error = sqlite3_errstr(sqlite3_extended_errcode(db->db));
    goto error;
  }

  if (SQLITE_OK != sqlite3_prepare_v2(db->db, delete_gesture_with_id_sql, -1,
                                      &db->delete_gesture_with_id_stmt, NULL)) {
    *error = sqlite3_errstr(sqlite3_extended_errcode(db->db));
    goto error;
  }
...
const char *database_add_gesture(Database *db, stroke_t *stroke,
  ...
  if (SQLITE_OK !=
      sqlite3_bind_int(db->insert_gesture_stmt, 1, (int)time(NULL)))
    return sqlite3_errstr(sqlite3_extended_errcode(db->db));
  if (SQLITE_OK != sqlite3_bind_text(db->insert_gesture_stmt, 2, description,
                                     -1, SQLITE_STATIC))
    return sqlite3_errstr(sqlite3_extended_errcode(db->db));
  if (SQLITE_OK !=
      sqlite3_bind_text(db->insert_gesture_stmt, 3, command, -1, SQLITE_STATIC))
    return sqlite3_errstr(sqlite3_extended_errcode(db->db));

  if (stroke->n < 0)
    return "stroke->n < 0";

  size_t n = sizeof(point) * (size_t)stroke->n;
  char bytes[n];
  ...
  if (SQLITE_OK != sqlite3_bind_blob(db->insert_gesture_stmt, 4, bytes, (int)n,
                                     SQLITE_STATIC))
    return sqlite3_errstr(sqlite3_extended_errcode(db->db));
...
static void _database_load_gesture(sqlite3_stmt *stmt, int *id,
  ...
  if (stroke) {
    ...
    if (n >= 0) {
      ...
      memcpy(stroke->p, bytes, (size_t)n);
      stroke->n = n / (int)sizeof(point);
    } else {
      stroke->n = 0;
    }
  }
  ...
  if (description)
    *description = strdup((const char *)sqlite3_column_text(stmt, 2));
  if (command)
    *command = strdup((const char *)sqlite3_column_text(stmt, 3));
}
  ...
  while (cb) {
    ...
    switch (status) {
      case SQLITE_DONE:
        ...
        return NULL;
      case SQLITE_ROW:
        _database_load_gesture(db->load_gestures_stmt, &id, &stroke,
                               &description, &command);
...
const char *database_load_gesture_with_id(Database *db, int id,
  ...
  _database_load_gesture(db->load_gesture_with_id_stmt, NULL, stroke,
                         description, command);
