...
void stroke_finish(stroke_t *s) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    s->p[i].alpha =
        atan2(s->p[i + 1].y - s->p[i].y, s->p[i + 1].x - s->p[i].x) / M_PI;
  }
}
...
double stroke_angle_difference(const stroke_t *a, const stroke_t *b,
...
void step(const stroke_t *a, const stroke_t *b, const int N, double *dist,
